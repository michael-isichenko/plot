...
* `pyplot`.  Uses `matplotlib.pyplot` supplied with python installation.  Requires the `--driver pyplot` option.  This backend blocks the calling shell process until the Qt window is closed.
